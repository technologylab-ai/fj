        ...
        .show => {
            ...
            const json_string = blk: {
                if (args.verbose) {
                    ...
                    switch (RecordType) {
                        fj_json.Client => break :blk std.fmt.allocPrint(self.arena,
                            \\ Client:
                            \\    shortname   : {s}
                            \\    company-name: {s}
                            \\    remarks     : {?s}
                        ...
                        fj_json.Rate => break :blk std.fmt.allocPrint(self.arena,
                            \\ Rate:
                            \\    shortname   : {s}
                            \\    hourly      : {d}
                            \\    daily       : {d}
    ...
    while (it.next()) |line_untrimmed| {
        ...
        if (std.mem.containsAtLeastScalar(u8, line, 1, ',')) {
            ...
            if (line.len >= 3) {
                ...
                try billables_writer.writer.print("\\groupheader{{{s}}}\n", .{current_group});
